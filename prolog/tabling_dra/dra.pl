...
init_dra(file0) :-current_prolog_flag(access_level,Was),asserta(was_access_level(Was)).
...
init_dra(1):- forall(property_pred(D,F) , make_db_pred(D,F)).
