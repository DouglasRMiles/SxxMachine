	...
	Operation bootpreds::PRED_close_2_static_exec(Prolog* engine)
	{
		...
		while(!tmp->isNil())
		{
			...
			if((car->isStructure()))
			{
				...
				{
					Term* bool_RenamedTODO = args[0]->dref();
					if(bool_RenamedTODO->equalsTerm(SYM_TRUE))
					{
						forceFlag = true;
					} else if(bool_RenamedTODO->equalsTerm(SYM_FALSE))
					{
						forceFlag = false;
					} else
					{
		...
		{
			PushbackReader* in_RenamedTODO = any_cast<PushbackReader*>(stream);
			if(in_RenamedTODO->equals(engine->getUserInput()))
			{
				return cont;
			}
			if(in_RenamedTODO->equals(engine->getCurrentInput()))
			{
				engine->setCurrentInput(engine->getUserInput());
			}
			try
			{
				in_RenamedTODO->close();
...
SymbolTerm* const  bootpreds::SYM_DOT = SYM(".");
	...
	Operation bootpreds::PRED_get_2_static_exec(Prolog* engine)
	{
		...
		try
		{
			PushbackReader* in_RenamedTODO = stream;
			int c = in_RenamedTODO->read();
	...
	Operation bootpreds::PRED_open_4_static_exec(Prolog* engine)
	{
		...
		try
		{
			if(a2->equalsTerm(SYM_READ))
			{
				...
				PushbackReader* in_RenamedTODO = new LineNumberPushbackReader(&tempVar, Prolog::PUSHBACK_SIZE);
				streamObject = TermData::FFIObject(in_RenamedTODO);
...
//JAVA TO C++ CONVERTER WARNING: The following method was originally marked 'synchronized':
	void bootpreds::writeDomainDefinitions(const wstring& folder, const wstring& packageName, Properties* domains)
	{
		...
		{
			// read the file first
			FileInputStream* in_RenamedTODO = nullptr;
			try
			{
				in_RenamedTODO = new FileInputStream(file);
				p->load(in_RenamedTODO);
			} catch(const IOException& e)
			{
				throw JavaException(e);
			} finally
