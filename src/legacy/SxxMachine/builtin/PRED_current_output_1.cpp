...
#include "PRED_current_output_1.h"
...
namespace SxxMachine
{
	...
	PRED_current_output_1::PRED_current_output_1(Term *a1, Operation cont)
	{
	LARG[0] = a1;
	this->cont = cont;
	}

	Operation PRED_current_output_1::exec(Prolog *engine)
	{
	...
	{
		FFIObjectTerm tempVar(engine->getCurrentOutput());
		a1->asVariablelTerm().bind(&tempVar, engine->trail);
	}
	else if ((dynamic_cast<FFIObjectTerm*>(a1) != nullptr))
	{
		FFIObjectTerm tempVar2(engine->getCurrentOutput());
