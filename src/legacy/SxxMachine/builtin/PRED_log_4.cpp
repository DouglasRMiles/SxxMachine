...
//	import static SxxMachine.PRED_loggable_1.LEVELS;
	...
	using P4 = SxxMachine::Predicate::P4;
	using Prolog = SxxMachine::Prolog;
	using StructureTerm = SxxMachine::StructureTerm;
	using SymbolTerm = SxxMachine::SymbolTerm;
	using Term = SxxMachine::Term;

	PRED_log_4::PRED_log_4(Term *a0, Term *a1, Term *a2, std::vector<Term*> &LARG, Operation cont)
	{
		this->LARG[0] = a0; //0];
		this->LARG[1] = a1; //1];
		this->LARG[2] = a2; //2];
	...
	Operation PRED_log_4::exec(Prolog *engine) throw(PrologException)
	{
