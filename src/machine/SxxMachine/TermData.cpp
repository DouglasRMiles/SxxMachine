...
namespace SxxMachine
{
	...
	SymbolTerm* TermData::F(const wstring& s, const int& i)
	{
	...
	DoubleTerm* TermData::Float(const double& i)
	{
...
std::vector<IntegerTerm*> TermData::negInts(NEG_INT_CACHE);
	...
	IntegerTerm* TermData::Integer(const int& i)
	{
		if(i < POS_INT_CACHE)
		{
			if(i < 0)
	...
	std::vector<Term*> TermData::VA(vector<Term> &array_RenamedTODO)
	{
		return array_RenamedTODO;
	}
	...
	bool TermData::isQuoted(const int& printingFlags)
	{
	...
	NumberTerm* TermData::NarrowDouble(const double& r)
	{
		NumberTerm* T;
		if(floor(r) == r)
		{
