...
namespace SxxMachine
{
	...
	bool VariableTerm::unifyInt(const int& i, Trail* trail)
	{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
	...
	Term* VariableTerm::copyImpl(IdentityHashMap<any, Term*>* copyHash, const int& deeply)
	{
		if(this->val->isVar())
		{
	...
	bool VariableTerm::equalsTerm(Term* obj, Comparator* comparator)
	{
	  if(isCyclic > 0)
	  {
		  return obj == val;
	...
	void VariableTerm::toStringImpl(const int& printFlags, StringBuilder* sb)
	{
	  toString();
	  if(isCyclic > 0)
	  {
	...
	Iterator<Term*>* VariableTerm::iterator(const bool& skipSyntax)
	{
