...
java::io::PrintWriter* const  Prolog::NO_OUTPUT = new java::io::PrintWriter(new WriterAnonymousInnerClass());
	...
	void Prolog::WriterAnonymousInnerClass::write(std::vector<char&>& cbuf, const int& off, const int& len) throw(IOException)
	{
...
java::io::PushbackReader* const  Prolog::NO_INPUT = new java::io::PushbackReader(new ReaderAnonymousInnerClass());
	...
	int Prolog::ReaderAnonymousInnerClass::read(std::vector<char&>& cbuf, const int& off, const int& len) throw(IOException)
	{
		return -1;
	}
...
SymbolTerm* const  Prolog::SYM_NECK = SymbolTerm::intern(":-", 2);
	...
	void Prolog::initOnce(InputStream* in_RenamedTODO, PrintStream* out, PrintStream* err)
	{
		...
		{
			BufferedReader tempVar(new InputStreamReader(in_RenamedTODO));
	...
	void Prolog::init(InputStream* in_RenamedTODO, PrintStream* out, PrintStream* err)
	{
		if(this->AREGS.empty())
		{
			initOnce(in_RenamedTODO,out,err);
		}
		this->stack->init();
		this->trail->init();
		...
		this->logger->init(this->stack->top_RenamedTODO);
		this->halt = 0;
		pendingGoals = Nil;
		assumptions = Nil;
		PENDING_INTERUPTS = 0;
	...
	void Prolog::pushCatcherB(const int& b)
	{
		this->catchersBindex++;
	...
	{
		ChoicePointFrame* top = this->stack->top_RenamedTODO;
	...
	Operation Prolog::jtry(const int& arity, Operation p, Operation next)
	{
	  this->trail->timeStamp = ++this->CPFTimeStamp;
	...
	{
		ChoicePointFrame* top = this->stack->top_RenamedTODO;
		top->restore_RenamedTODO.accept(top,this);
	...
	{
		ChoicePointFrame * const top = this->stack->top_RenamedTODO;
		top->restore_RenamedTODO.accept(top,this);
	...
	void Prolog::setCurrentInput(PushbackReader* in_RenamedTODO)
	{
		this->currentInput = in_RenamedTODO;
	}
