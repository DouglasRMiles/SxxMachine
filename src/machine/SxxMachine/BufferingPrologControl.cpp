...
namespace SxxMachine
{
	...
	bool BufferingPrologControl::execute(const wstring& pkg, const wstring& functor, vector<Term> &args)
	{
	  return once(pkg, functor, { args })->size() > 0;
	}

	Term* BufferingPrologControl::once(const wstring& pkg, const wstring& functor, Term* arg)
	{
	...
	bool BufferingPrologControl::run(const int& newLimit)
	{
