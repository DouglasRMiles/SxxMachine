...
namespace SxxMachine
{
	...
	bool Token::isSolo(const int& c)
	{
		return (c == '!' || c == ';');
	}

	bool Token::isSymbol(const int& c)
	{
		switch(c)
		{
		case '+':
	...
	int Token::read_token(StringBuilder* s, PushbackReader* in_RenamedTODO) throw(IOException)
	{
	int c, c1;
	int rc;
	c = in_RenamedTODO->read(); // get 1st. char
	if(c == -1)
	{
	...
	{
		return read_number(c, s, in_RenamedTODO);
	}
	if(islower(static_cast<char>(c)))
	{
		rc = read_word(c, s, in_RenamedTODO);
		if(rc > 0)
		{
		rc = 'A';
		}
	...
	case '.': // full stop or symbol
		c1 = in_RenamedTODO->read();
		if(c1 == -1 || c1 <= ' ')
		{
		s->append(".");
		return '.';
		}
		in_RenamedTODO->unread(c1);
		break;
	case '%': // one line comment
		s->append(" ");
		while((c1 = in_RenamedTODO->read()) != '\n')
		{
		if(c1 == -1)
		{
			return ' ';
	...
	case '/': // start of comment or symbol
		...
		break;
	case '\'':
		rc = read_quoted(c, s, in_RenamedTODO);
		if(rc > 0)
		{
		rc = 'A';
		}
	...
	case '-':
	{
		...
		if(isdigit(static_cast<char>(c1)))
		{
			...
			rc = read_number(c1, s1, in_RenamedTODO);
			try
			{
		...
		in_RenamedTODO->unread(cbuf);
		rc = read_symbol(c, s, in_RenamedTODO);
		if(rc > 0)
		{
			rc = 'A';
		}
	...
	int Token::read_number(const int& c, StringBuilder* s, PushbackReader* in_RenamedTODO) throw(IOException)
	{
		int c1, c2, c3;
		in_RenamedTODO->unread(c);
		for(c1 = in_RenamedTODO->read(); isdigit(static_cast<char>(c1)); c1 = in_RenamedTODO->read())
		{
		...
		c2 = in_RenamedTODO->read();
		if(!isdigit(static_cast<char>(c2)))
		{
			in_RenamedTODO->unread(c2);
			in_RenamedTODO->unread(c1);
			return number_type(s);
		}
		s->append(static_cast<char>(c1));
		...
		if(c2 == '-' || c2 == '+')
		{
			c3 = in_RenamedTODO->read();
			if(!isdigit(static_cast<char>(c3)))
			{
				in_RenamedTODO->unread(c3);
				in_RenamedTODO->unread(c2);
				in_RenamedTODO->unread(c1);
				return TOKEN_DOUBLE;
			}
	...
	int Token::read_word(const int& c, StringBuilder* s, PushbackReader* in_RenamedTODO) throw(IOException)
	{
	int c1;
	in_RenamedTODO->unread(c);
	for(;;)
	...
	int Token::read_quoted(const int& quote, StringBuilder* s, PushbackReader* in_RenamedTODO) throw(IOException)
	{
	int rc;
	int c1;
	for(;;)
	{
		...
		{
		rc = escapeSequences(c1, s, in_RenamedTODO);
		if(rc > 0)
		{
			continue;
		} else
	...
	int Token::escapeSequences(const int& backslash, StringBuilder* s, PushbackReader* in_RenamedTODO) throw(IOException)
	{
	int c;
	c = in_RenamedTODO->read();
	switch(c)
	{
	case 'b': // backspace
	...
	int Token::read_symbol(const int& c, StringBuilder* s, PushbackReader* in_RenamedTODO) throw(IOException)
	{
	int c1;
	s->append(static_cast<char>(c));
	//	in.unread(c);
