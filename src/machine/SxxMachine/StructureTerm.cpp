...
namespace SxxMachine
{
	...
	{
		return name_Renamed == "," && arityOrType() == 2;
	}
	...
	StructureTerm::StructureTerm(const wstring &name, int arity) : StructureTerm(name, makeVars(arity))
	{
	}

	std::vector<Term*> StructureTerm::makeVars(int arity)
	{
		// TODO Auto-generated method stub
	...
	void StructureTerm::init(int arity)
	{
	...
	Term *StructureTerm::ArgDeRef(int i)
	{
		return argz[i]->dref();
	}

	Term *StructureTerm::ArgNoDeRef(int i)
	{
		return argz[i];
	}

	int StructureTerm::getIntArg(int i)
	{
		return static_cast<int>(Expect::asInt(ArgDeRef(i))->doubleValue());
	}

	void StructureTerm::setArg(int i, Term *T)
	{
		argz[i] = T;
	}

	int StructureTerm::unifyArg(int i, Term *T, Prog *p)
	{
	...
	StructureTerm *StructureTerm::toClone()
	{
		...
		if (functor_Renamed != nullptr)
		{
			return new StructureTerm(functor_Renamed, argz);
		}
		f = new StructureTerm(name_Renamed, argz);
	...
	StructureTerm *StructureTerm::S(const wstring &getName, int arity)
	{
		// TODO Auto-generated method stub
	...
	{
		return this->functor_Renamed != nullptr ? this->functor_Renamed : (this->functor_Renamed = SymbolTerm::intern(this->name_Renamed, this->argz.size()));
	}
	...
	wstring StructureTerm::name()
	{
		if (name_Renamed == "")
		{
			name_Renamed = functor_Renamed->name();
			//  name = name.intern();
		}
		return name_Renamed;
	}

	Term *StructureTerm::arg0(int nth)
	{
		if (isConsOL())
		{
	...
	bool StructureTerm::unifyImpl(Term *t, Trail *trail)
	{
		...
		if (this->name_Renamed != "")
		{
			if (this->name_Renamed != st->name())
			{
				return false;
			}
		}
		...
		{
			if (!this->functor_Renamed->unify(st->functor(), trail))
			{
				return false;
			}
		}
	...
	Term *StructureTerm::copyImpl(IdentityHashMap<any, Term*> *copyHash, int deeply)
	{
		if (isConsOL())
		{
		...
		if (cyclic)
		{
			if (this->functor_Renamed != nullptr)
			{
				return new StructureTerm(this->functor_Renamed, a);
			}
	...
	int StructureTerm::containsTermImpl(Term *variableTerm, Comparator *comparison)
	{
		...
		if (functor_Renamed != nullptr)
		{
			return functor_Renamed->containsTerm(variableTerm, comparison) + contains;
		}
		return contains;
	}

	bool StructureTerm::equalsTerm(Term *obj, OpVisitor *comparator)
	{
		if (isConsOL())
		{
		...
		{
			if (!this->functor_Renamed->equalsTerm(st->functor(), comparator))
			{
				return false;
			}
		}
	...
	void StructureTerm::toStringImpl(int printingFlags, StringBuilder *sb)
	{
		if (isCons())
		{
		...
		if (this->functor_Renamed != nullptr)
		{
			functor_Renamed->toQuotedString(printingFlags, sb);
		}
		else
		{
			if (!isQuoted(printingFlags))
			{
				sb->append(name_Renamed);
			}
			else
			{
				Token::toQuotedString(name_Renamed, sb);
			}
		}
		sb->append('(');
	...
	void StructureTerm::toListStringImpl(int printingFlags, StringBuilder *sb)
	{
		Term *x = this;
		sb->append("[");
		for (;;)
	...
	Iterator<Term*> *StructureTerm::listIterator(bool includeSyntax)
	{
		return new ListTermIterator(this, includeSyntax);
	}

	Iterator<Term*> *StructureTerm::iterator(bool includeSyntax)
	{
		if (isConsOL())
		{
...
SymbolTerm *const StructureTerm::StructureTermIterator::LEFT_BRACKET = SymbolTerm::intern("(");
	...
	StructureTerm::StructureTermIterator::StructureTermIterator(Term *functor, std::vector<Term*> &args, bool includeSyntax) : functorIt(functor), argsIt(args), length(args.size())
	{
	...
	void StructureTerm::setarg0(Trail *trail, int i, Term *val)
	{
		if (isConsOL())
		{
