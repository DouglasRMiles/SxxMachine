...
SymbolTerm* DataBase::no = Prolog::aNo;
	...
	Term* DataBase::out(const wstring& k, Term* pattern, const bool& copying)
	{
	...
	void DataBase::all0(const int& max, vector To, const wstring& k, Term* FXs)
	{
		if(0 == max)
		{
			max = -1;
	...
	Term* DataBase::all1(const int& max, Term* FXs)
	{
	...
	Term* DataBase::all2(const int& max, const wstring& k, Term* FXs)
	{
		if(k == "")
		{
	...
	bool DataBase::fromFile(const wstring& f, const bool& overwrite)
	{
...
wstring DataBase::lastFile = Init::default_lib;
	...
	bool DataBase::fileToProg(const wstring& fname, const bool& overwrite)
	{
	...
	bool DataBase::streamToProg(Reader* sname, const bool& overwrite)
	{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
	...
	bool DataBase::streamToProg(const wstring& fname, Reader* sname, const bool& overwrite)
	{
		try
		{
