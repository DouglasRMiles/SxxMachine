...
namespace SxxMachine
{
	...
	Clause::Clause(Term* s, std::vector<Term*>& args, HashDict* dict, const bool& ground, const wstring& fname, const int& begins_at, const int& ends_at) : StructureTerm(s,args)
	{
		this->dict = dict;
		this->ground = ground;
		this->fname = fname;
	...
	Clause* Clause::toClone()
	{
	  ...
	  e->printStackTrace();
	  return new Clause(functor_RenamedTODO,argz, dict, ground, fname, begins_at, ends_at);
	}
	}
	...
	void Clause::setFile(const wstring& fname, const int& begins_at, const int& ends_at)
	{
	...
	wstring Clause::pprint(const bool& replaceAnonymous)
	{
	...
	Clause* Clause::cnumbervars(const bool& replaceAnonymous)
	{
		if(dict == nullptr)
		{
