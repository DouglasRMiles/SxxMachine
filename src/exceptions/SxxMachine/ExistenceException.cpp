...
SymbolTerm* const  ExistenceException::EXISTENCE_ERROR = SymbolTerm::intern("existence_error", 5);
	...
	ExistenceException::ExistenceException(Operation _goal, const int& _argNo, const wstring& _objType, Term* _culprit, const wstring& _message) : objType(_objType), culprit(_culprit), message(_message.intern())
	{
	this->goal = _goal;
	this->argNo = _argNo;
	}
