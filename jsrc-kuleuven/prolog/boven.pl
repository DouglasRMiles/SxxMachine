...
isIndexingAllowed(ClassName) :- concat_atom(['DISABLE_INDEXING_', ClassName], FullName), b_getval(FullName, true), !, fail.
isIndexingAllowed(_).
...
constructterm(cut(_,C),Strings) :- ! ,
		...
		writel(['new Funct(getConsts().string',I,', new HeapChoice(mach.getCUTB()),',
...
constructterm(Term,Strings) :-
			...
			getnameindex(Strings,Name,0,I) ,
			writel(['new Funct(getConsts().string',I,',',
