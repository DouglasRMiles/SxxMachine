...
public abstract class PrologControl {
  ...
  public PrologClassLoader getPrologClassLoader() {
    if (this.engine.pcl == null) this.engine.pcl = new PrologClassLoader();
  ...
  public void setMaxArity(int max) {
    if (max < 8) throw new IllegalStateException("invalid arity " + max);
    if (this.engine.AREGS != null)
      throw new IllegalStateException("Prolog already initialized");
  ...
  public void configureUserIO(InputStream in, OutputStream out,
    ...
    if (in != null) {
      this.engine.streamManager.put(SymbolTerm.intern("user_input"),
          TermData.FFIObject(new PushbackReader(new BufferedReader(new InputStreamReader(in)),
              Prolog.PUSHBACK_SIZE)));
    }
    if (out != null) {
      this.engine.streamManager.put(SymbolTerm.intern("user_output"),
          TermData.FFIObject(new PrintWriter(
              new BufferedWriter(new OutputStreamWriter(out)), true)));
    }
    if (err != null) {
      this.engine.streamManager.put(SymbolTerm.intern("user_error"),
          TermData.FFIObject(new PrintWriter(
              new BufferedWriter(new OutputStreamWriter(err)), true)));
    }
  }
  ...
  public void executePredicate(boolean isOutter)
    ...
    try {
      ...
      mainLoop: 
        do {
          try {
            do {
              if(isOutter) {code = insertPendingGoals(code);}
              if(!Prolog.BE_SAFE)logger.beforeExec(code);
              if(code==null) {
                break;
              }
              engine.pred = code;
      ...
      if(isOutter)  { this.code = insertPendingGoals(code);}
      else {
        this.code = code;  
      }
