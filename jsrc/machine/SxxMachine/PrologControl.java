...
public abstract class PrologControl {
	...
	public void configureUserIO(InputStream in, OutputStream out, OutputStream err) {
		...
		if (in != null) {
			this.engine.streamManager.put(SymbolTerm.intern("user_input"), TermData.FFIObject(
					new PushbackReader(new BufferedReader(new InputStreamReader(in)), Prolog.PUSHBACK_SIZE)));
		}
		if (out != null) {
			this.engine.streamManager.put(SymbolTerm.intern("user_output"),
					TermData.FFIObject(new PrintWriter(new BufferedWriter(new OutputStreamWriter(out)), true)));
		}
		if (err != null) {
			this.engine.streamManager.put(SymbolTerm.intern("user_error"),
					TermData.FFIObject(new PrintWriter(new BufferedWriter(new OutputStreamWriter(err)), true)));
		}
	}
