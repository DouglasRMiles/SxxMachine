...
public abstract class Term extends KPTerm implements Comparable<Term> {
   ...
   final public int termHashCode() {
      if(loopedHashCodeTerm>0) return 1;
      try {
        loopedHashCodeTerm++;
   ...
   //  public Term val = this;
    ...
    final public boolean unify(Term t, Trail trail) {
      ...
      if(loopedUnifyTerm>0) return true;
      try {
        loopedUnifyTerm++;
    ...
    public boolean isNil() { return Prolog.Nil.equalsTerm(this, StrictEquals); }
    /**
     ...
     * @see FFIObjectTerm
     */
    public boolean isFFIObject() { return false; }
    /**
    ...
    public Term[] args(){ throw new ArrayIndexOutOfBoundsException(666); }
    public Term arg0(int nth) { throw new ArrayIndexOutOfBoundsException(nth); }
    /** @return get the nth argument of {@link #isStructure()} or {@link #isCons()}. */
    public Term nth0(int nth) { throw new ArrayIndexOutOfBoundsException(nth); }
	public Term car() { throw new ArrayIndexOutOfBoundsException(-1); }
	public Term cdr() { throw new ArrayIndexOutOfBoundsException(-2); }
	public int length() { throw new ArrayIndexOutOfBoundsException(-3); }
    public ListTerm add(Term t) { throw new ArrayIndexOutOfBoundsException(-5); }
    public ListTerm append(Term t) { throw new ArrayIndexOutOfBoundsException(-5); }
    /**
    ...
    public static boolean instanceOfTerm(Object obj0) {
    	if(!(obj0 instanceof Term)) return false;
    	Term obj = (Term) obj0;
	return obj .isVar() ||
	    obj .isInteger() ||
	    obj .isLong() ||
 ...
 public Term drefAttrs() {
    // TODO Auto-generated method stub
	  if(fdata!=null) return this;
      Term attterm = dref();
      if(attterm==this)return attterm;
  ...
  public Term ArgNoDeRef(int i) {
	...
	public Term findOrAttrValue(Trail trail, boolean createIfMissing, Term name) {
	    Term drefAttrs = drefAttrs();
	    if (drefAttrs!=this) return drefAttrs.findOrAttrValue(trail, createIfMissing, name);
  ...
  public Term freeze(Trail trail, Term newval) {
    Term drefAttrs = drefAttrs();
    if (drefAttrs!=this) return drefAttrs.freeze(trail, newval);
	...
	public Term frozenGoals() {
      Term drefAttrs = drefAttrs();
      if (drefAttrs!=this) return drefAttrs.frozenGoals();
	   return nullIs(this.getGoals(), Prolog.True);
	}

	public boolean isAttvar() {
      Term drefAttrs = drefAttrs();
      if (drefAttrs!=this) return drefAttrs.isAttvar();
	...
	public boolean isFrozen() {
      Term drefAttrs = drefAttrs();
      if (drefAttrs!=this) return drefAttrs.isFrozen();
	  if (fdata==null) return false;
  ...
  public void setGoals(Trail trail, Term newval) {
    Term drefAttrs = drefAttrs();
    if (drefAttrs!=this) { drefAttrs.setGoals(trail, newval);
     return;
    }
  ...
  public Term setGoals(Term newval) {
    Term drefAttrs = drefAttrs();
    if (drefAttrs!=this) return drefAttrs.setGoals(newval);
  return getFdata().setGoals(newval);
}

private Term getGoals() {
  Term drefAttrs = drefAttrs();
  if (drefAttrs!=this) return drefAttrs.getGoals();
  if(fdata==null) return Prolog.True;
  return getFdata().getGoals();
}

public void setAttrs(Term newval) {
  Term drefAttrs = drefAttrs();
  if (drefAttrs!=this) { drefAttrs.setAttrs(newval);
  return;
  }
...
public Term getAttrs() {
  Term drefAttrs = drefAttrs();
  if (drefAttrs!=this) return drefAttrs.getAttrs();
  if(fdata==null) return Prolog.Nil;
  ...
  public boolean isFVar() {
    Term drefAttrs = drefAttrs();
    if (drefAttrs!=this) return drefAttrs.isFVar();
  ...
  public FrozenData getFdata() {
    if(fdata==null) fdata = new FrozenData();
    return fdata;
  }
