 ...
 * @version 1.0
 */
public class StructureTerm extends ListTerm implements Cloneable {


  public String getKey() {
      if(true) return super.getKey();
...
//  }
  ...
  */
  ...
  public StructureTerm toClone() {
      ...
      if(functor!=null) return new StructureTerm(functor,argz);
  ...
  public boolean isAtomicValue() {
    ...
    public StructureTerm(String name, Term... _args){
      ...
      super(name);
    	this.argz = _args;
    	if(!Prolog.BE_FAST)checkImmuatble();
    }
    ...
    public Object toJava() { 
      if(isConsOL()) return super.toJava();
      return this;
    }
    /**
     * Constructs a new Prolog compound term
    ...
    public boolean convertible(Class type) { 
        if (isConsOL()) return convertible(List.class, type); 
        return true;
    }
     ...
     * @see #functor
     */
    public final Term functor(){ return this.functor!=null?this.functor:
       (this.functor=SymbolTerm.intern(this.name, this.argz.length)); }
  ...
  public final boolean unifyImpl(Term t, Trail trail) {
	...
	protected Term copyImpl(IdentityHashMap<Object, Term> copyHash, int deeply) {
	  if (isConsOL()) return super.copyImpl(copyHash, deeply);
		if (this.immutable){
			return this;
		}
		...
		if(cyclic) {
	        if(this.functor!=null) return new StructureTerm(this.functor, a);	        
    ...
    public int containsTermImpl(Term variableTerm, Comparator comparison) {
      ...
      if( functor!=null) return functor.containsTerm(variableTerm, comparison)+contains;
      return contains;
    }
  ...
  public int termHashCodeImpl() {
	  if (isConsOL()) return super.termHashCodeImpl();
        int h = 1;
  ...
  public Iterator<Term> listIterator(boolean includeSyntax) {
	...
	public Iterator<Term> iterator(boolean includeSyntax) {
	  if(isConsOL()) return listIterator(includeSyntax);
	...
	public int compareTo(Term anotherTerm) { // anotherTerm must be dereferenced.
      if(isConsOL()) return compareTo(anotherTerm);
