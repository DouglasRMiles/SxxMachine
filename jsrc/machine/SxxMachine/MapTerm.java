...
public class MapTerm extends SystemObject {
  ...
  public int containsTermImpl(Term variableTerm, Comparator comparison) {
     ...
     */
    @Override
    public final Term car() { return this.argz[0]; }
    ...
    public final boolean unifyImpl(Term t, Trail trail) {
		Term p = this;
		t = t.dref();
		while ((t .isMap()) && (p .isMap())
				&& (  p).car().unify((  t).car(), trail)) {
			p = (  p).cdr().dref();
			t = (  t).cdr().dref();
		}
		if (t .isVar()) {
        ...
        if (t .isMap()) {
          if(p.isAtomicValue()) return false;
          return t.unify(p, trail);
        }
        if (p .isMap()) {
          if(t.isAtomicValue()) return false;
  ...
  public int termHashCodeImpl() {
	...
	public MapTerm join(Term term) {
	  ...
	  if(cdr.isMap()) return cdr.join(term);
	  if(cdr==Prolog.Nil) {
        // proper list
